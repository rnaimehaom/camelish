...
class ChemcentralCreate  {
  ...
  constraint fk_sp2structures FOREIGN KEY (structure_id) references ''' + props.schema + ''' .structures(cd_id) ON DELETE CASCADE
                ...
                Utils.execute(db, 'seeding categories', 'insert into ' + props.schema + '''.categories (category_name) values
('ACTIVITY_DATA'),('CALC_PROP'),('PHYSCHEM')''')
    
                Utils.execute(db, 'seeding sources', 'insert into ' + props.schema + '''.sources (category_id, source_name, source_description, type, active) 
